...
void generate_cylinder(uint32 long_lines, array<world_vertex> *vertices)
{
    ...
    for(uint32 i = 0; i < long_lines; i++, a += da)
        points.push_back(vector<float32, 3>(cos(a), 0.0f, sin(a)));
    for(uint32 i = 0, prev_i = long_lines - 1; i < long_lines; i++, prev_i = i - 1)
    {
        vertices->push_back(world_vertex(points[prev_i]));
        vertices->push_back(world_vertex(points[i]));
        vertices->push_back(world_vertex(vector<float32, 3>(0.0f, 0.0f, 0.0f)));
        vertices->push_back(world_vertex(vector<float32, 3>(points[i].x, 1.0f, points[i].z)));
        vertices->push_back(world_vertex(vector<float32, 3>(points[prev_i].x, 1.0f, points[prev_i].z)));
        vertices->push_back(world_vertex(vector<float32, 3>(0.0f, 1.0f, 0.0f)));
        vertices->push_back(world_vertex(vector<float32, 3>(points[i].x, 0.0f, points[i].z)));
        vertices->push_back(world_vertex(vector<float32, 3>(points[prev_i].x, 0.0f, points[prev_i].z)));
        vertices->push_back(world_vertex(vector<float32, 3>(points[prev_i].x, 1.0f, points[prev_i].z)));
        vertices->push_back(world_vertex(vector<float32, 3>(points[prev_i].x, 1.0f, points[prev_i].z)));
        vertices->push_back(world_vertex(vector<float32, 3>(points[i].x, 1.0f, points[i].z)));
        vertices->push_back(world_vertex(vector<float32, 3>(points[i].x, 0.0f, points[i].z)));
