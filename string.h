...
template<typename char_type> struct string
{
	...
	string(const string &str)
	{
		...
		size = 0;
		capacity = 0;
		insert_range(0, str.addr, str.addr + str.size);
	}
	...
	void push_back(const char_type &value)
	{
		if(size == capacity) increase_capacity(1);
		addr[size] = value;
		size++;
	...
	/*Get reference to element of array by idx
	idx must be valid array position*/
	char_type operator[](uint64 idx) const
	{
		return addr[idx];
	}
	...
	{
		if(value == 0) push_back(char_type('0'));
