    ...
    switch(msg)
    {
		...
		case WM_MOUSEWHEEL:
		{
            if(GET_WHEEL_DELTA_WPARAM(wParam) > 0)
                mouse()->roll_direction = mouse_wheel_roll_direction::forward;
            else mouse()->roll_direction = mouse_wheel_roll_direction::back;
			mouse()->mouse_wheel_rotate.trigger();
			wnd->fm.mouse_wheel_rotate.trigger();
			wnd->update();
			break;
		}
		case WM_MOVE:
...
bool os_load_glyph(character_data *data)
{
#ifdef _WIN32
