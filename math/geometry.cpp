...
void geometry_path::push_elliptic_arc(vector<float32, 2> point, float32 rx, float32 ry, float32 begin_angle, float32 end_angle, float32 rotation)
{
	geometry_path_data unit;
	...
	unit.p1 = point;
	vector<float32, 2> n1 = vector<float32, 2>(cos(end_angle), sin(end_angle)),
		n2 = vector<float32, 2>(cos(rotation), sin(rotation));
	unit.p2.x = unit.p1.x - unit.rx * n1.x * n2.x + unit.ry * n1.y * n2.y;
	unit.p2.y = unit.p1.y - unit.rx * n1.x * n2.y - unit.ry * n1.y * n2.x;
	data.push_back(unit);
}
...
{
	if(2.0f * rrect.rx > rrect.rect.extent.x || 2.0f * rrect.ry > rrect.rect.extent.y) return;
	...
	push_elliptic_arc(vector<float32, 2>(rrect.rect.position.x + rrect.rect.extent.x, rrect.rect.position.y + rrect.ry),
		rrect.rx, rrect.ry, 1.5f * pi, 0.0f, 0.0f);
	...
	push_elliptic_arc(vector<float32, 2>(rrect.rect.position.x + rrect.rect.extent.x - rrect.rx, rrect.rect.position.y + rrect.rect.extent.y),
		rrect.rx, rrect.ry, 0.0f, 0.5f * pi, 0.0f);
	...
	push_elliptic_arc(vector<float32, 2>(rrect.rect.position.x, rrect.rect.position.y + rrect.rect.extent.y - rrect.ry),
		rrect.rx, rrect.ry, 0.5f * pi, pi, 0.0f);
	...
	push_elliptic_arc(vector<float32, 2>(rrect.rect.position.x + rrect.rx, rrect.rect.position.y),
		rrect.rx, rrect.ry, pi, 1.5f * pi, 0.0f);
}
