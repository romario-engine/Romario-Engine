...
void http_response::compose(array<byte> *target)
{
	for(uint64 i = 0; i < version.size; i++)
		target->push_back(byte(version.addr[i]));
	target->push_back(byte(U' '));
	for(uint64 i = 0; i < status_code.size; i++)
		target->push_back(byte(status_code.addr[i]));
	target->push_back(byte(U' '));
	for(uint64 i = 0; i < status_message.size; i++)
		target->push_back(byte(status_message.addr[i]));
	target->push_back(byte(U'\r'));
	target->push_back(byte(U'\n'));
	if(content_type.size != 0)
	{
		...
		for(uint64 i = 0; i < content_type.size; i++)
			target->push_back(byte(content_type.addr[i]));
		target->push_back(byte(U'\r'));
		target->push_back(byte(U'\n'));
	}
	if(data.size != 0)
	{
		...
		for(uint64 i = 0; i < content_length.size; i++)
			target->push_back(byte(content_length.addr[i]));
	}
	target->push_back(byte(U'\r'));
	target->push_back(byte(U'\n'));
	target->push_back(byte(U'\r'));
