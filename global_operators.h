...
template<typename function_type> function_type *emit_empty_function()
{
...
/*Clear <target> content and copy <source> content into <target>.*/
template<typename value_type>
void copy(const value_type &source, value_type *target)
{
	target->~value_type();
	new(target) value_type(source);
}

/*Mark value as moving*/
template<typename value_type>
value_type &&move_ref(const value_type &value)
{
	return static_cast<value_type &&>(const_cast<value_type &>(value));
}

/*Clear <target> content and move <source> content into <target>.*/
template<typename value_type>
void move(const value_type &source, value_type *target)
{
	target->~value_type();
	new(target) value_type(move_ref(source));
}

/*Swap content with move method.*/
template<typename value_type>
